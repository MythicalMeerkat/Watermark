...
base_images_folder_path = input("Enter the path to your base image(s) folder: ")
...
print("\nBase Image(s) Directory Found Successfully: ", base_images_folder_path, "\n")
...
request = input("Your base image(s) and watermark template MUST BE THE SAME SIZE, if you choose to add one. "
                "Would you like to resize your base image(s)[Y/N]: ")

if request == "Y" or request == "y":
    resize_request = True
    request = None
    print("Script will resize image(s).")
else:
    print("Script will not resize image(s).")

# Does the user want to keep their resized images once the script is over?

request = input("\nWould you like to DELETE the resized image(s) once the script is finished[Y/N]: ")
if request == "Y" or request == "y":
        delete_request = True
...
request = input("\nWould you like to add a watermark to your image(s)[Y/N]: ")
if request == "Y" or request == "y":
    ...
    print("Image(s) will not be watermarked.")
...
if resize_request or watermark_request:
    directory_name = input("Enter a directory name for the image(s). "
...
if resize_request and watermark_request:
    ...
    print("\nBASE IMAGE(s) DIRECTORY: ", base_images_folder_path)
    request = None
    request = input("Are you sure you want to add a watermark to your image(s) "
    ...
    if request == "Y" or request == "y":
        ...
        start = resizing_functions.return_start_time()
        end = time.time()
    else:
...
if resize_request and not watermark_request:
    print("\nBASE IMAGE(s) DIRECTORY: ", base_images_folder_path)
    request = None
    request = input("Are you sure you want to resize your image(s) "
                    "located in the above base directory[Y/N]: ")
    if request == "Y" or request == "y":
        watermark_images_save_path = resizing_functions.resize_images(base_images_folder_path, directory_name)
        request = None
...
if watermark_request and not resize_request:
    print("\nBASE IMAGE(s) DIRECTORY: ", base_images_folder_path)
    request = None
    request = input("Are you sure you want to watermark your image(s) "
                    "located in the above base directory[Y/N]: ")
    if request == "Y" or request == "y":
